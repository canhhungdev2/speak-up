import { Component, ChangeDetectionStrategy, signal, inject, Input, Output, EventEmitter } from '@angular/core';
  ...
  template: `
    <div class="min-h-screen bg-gray-50 dark:bg-[#0f172a] flex transition-colors duration-500 font-sans">
      ...
      <aside [class.translate-x-0]="isMobileMenuOpen()" 
             [class.w-72]="!isSlimSidebar()"
             [class.w-20]="isSlimSidebar()"
             [class.p-8]="!isSlimSidebar()"
             [class.p-4]="isSlimSidebar()"
             class="fixed md:sticky top-0 left-0 h-screen bg-white dark:bg-[#1e293b]/50 dark:backdrop-blur-xl border-r border-gray-200 dark:border-white/5 flex flex-col z-[100] transition-all md:translate-x-0 -translate-x-full">
        
        <!-- Logo -->
        <div class="flex items-center gap-3 mb-10" [class.justify-center]="isSlimSidebar()">
          ...
          <span *ngIf="!isSlimSidebar()" class="text-2xl font-black text-gray-900 dark:text-white tracking-tighter font-outfit whitespace-nowrap">SpeakUp</span>
        </div>

        <!-- Sidebar Toggle Button (Desktop) -->
        <button (click)="toggleSlim()" 
                class="hidden md:flex absolute -right-4 top-12 w-8 h-8 bg-white dark:bg-[#1e293b] border border-gray-100 dark:border-white/10 rounded-full items-center justify-center shadow-sm hover:text-primary transition-all z-[110]">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transition-transform duration-300" 
               [class.rotate-180]="isSlimSidebar()"
        ...
        <div class="space-y-8 flex-grow overflow-y-auto pr-2 scrollbar-hide">
          <div>
            <p *ngIf="!isSlimSidebar()" class="text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-6 px-4">Menu Chính</p>
            <nav class="space-y-2">
              <a routerLink="/learner" routerLinkActive="bg-primary/10 text-primary dark:bg-primary/20" [routerLinkActiveOptions]="{exact: true}"
                 [title]="isSlimSidebar() ? 'Dashboard' : ''"
                 ...
                 [class.justify-center]="isSlimSidebar()">
                 ...
                 <span *ngIf="!isSlimSidebar()" class="whitespace-nowrap">Dashboard</span>
              </a>
              <a routerLink="/learner/courses" routerLinkActive="bg-primary/10 text-primary dark:bg-primary/20"
                 [title]="isSlimSidebar() ? 'Khóa học' : ''"
                 class="flex items-center gap-4 px-4 py-3.5 text-gray-500 dark:text-slate-400 font-bold rounded-2xl hover:bg-gray-100 dark:hover:bg-white/5 transition-all group"
                 [class.justify-center]="isSlimSidebar()">
                 <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 group-hover:scale-110 transition-transform shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                   <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                 </svg>
                 <span *ngIf="!isSlimSidebar()" class="whitespace-nowrap">Khóa học</span>
              </a>
              <a routerLink="/learner/vocabulary" routerLinkActive="bg-primary/10 text-primary dark:bg-primary/20"
                 [title]="isSlimSidebar() ? 'Từ vựng' : ''"
                 class="flex items-center gap-4 px-4 py-3.5 text-gray-500 dark:text-slate-400 font-bold rounded-2xl hover:bg-gray-100 dark:hover:bg-white/5 transition-all group"
                 [class.justify-center]="isSlimSidebar()">
                 <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 group-hover:scale-110 transition-transform shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                   <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                 </svg>
                 <span *ngIf="!isSlimSidebar()" class="whitespace-nowrap">Từ vựng</span>
              </a>
            </nav>
          </div>

          <div *ngIf="!isSlimSidebar()">
            ...
            <nav class="space-y-2">
              <a href="#" class="flex items-center gap-4 px-4 py-3.5 text-gray-500 dark:text-slate-400 font-bold rounded-2xl hover:bg-gray-100 dark:hover:bg-white/5 transition-all">
                 ...
                 <span class="whitespace-nowrap">Diễn đàn</span>
              </a>
            </nav>
          </div>
        </div>
        ...
        <div class="pt-8 border-t border-gray-100 dark:border-white/5 space-y-4">
           <button (click)="themeService.toggleTheme()" 
                   [title]="isSlimSidebar() ? 'Đổi chủ đề' : ''"
                   class="w-full flex items-center p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/10 transition-all group"
                   [class.justify-center]="isSlimSidebar()">
             <span *ngIf="!isSlimSidebar()" class="text-sm font-bold text-gray-600 dark:text-slate-300 flex-grow text-left">Chế độ tối</span>
           ...
           <div class="flex items-center gap-4 p-2" [class.justify-center]="isSlimSidebar()">
      ...
      <div class="flex-grow flex flex-col min-w-0">
        ...
        <header class="h-24 px-8 flex justify-between items-center bg-white/80 dark:bg-[#0f172a]/80 backdrop-blur-md sticky top-0 z-[50] border-b border-gray-100 dark:border-white/5">
           <!-- Search Bar -->
           <div *ngIf="!isSlimSidebar()" class="relative w-full max-w-md hidden md:block">
           ...
           <div *ngIf="isSlimSidebar()" class="flex items-center gap-3">
...
export class LearnerLayoutComponent {
  @Input() set isSlim(value: boolean) {
    this.isSlimSidebar.set(value);
  }
  @Input() title = '';
